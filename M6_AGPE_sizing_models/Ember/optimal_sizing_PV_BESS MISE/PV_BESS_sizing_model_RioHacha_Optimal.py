...
#    Colombian Electricity Market      
     ...
     'Ppvu'    : 'PpvuRioHachaERAS20052023_localtime.inc',  # Profile Solar RioHacha ERAS 2005-2023     
    "periodo" : "periodo.inc",  # c
# ----------------------
}
...
er = 0 # Prices not included in this analysis
kappa = [0, 0, 0, 0, 0, 0, 0] 

Plinst  = 2                         # kW peak/installed Load DataCenter
...
Area   = 0.2143                      # Area module 30W $65\text{ cm} \times 33\text{ cm}$ or $54\text{ cm} \times 40\text{ cm}$
eta    = 0.14                      # PV module efficiency
eff_c  = 0.95                      # BESS charge inverter efficiency
eff_d  = 0.95                      # BESS discharge inverter efficiency
...
#m.addConstr(C == 38, name='r10') 
#m.addConstr(PinverterBESS == 10.1, name='r11')
#m.addConstr(Ppvinst    == 10.1, name='r12')
...
if m.Status == GRB.OPTIMAL:
    ...
    print(f"Npan: {nx.X:,.2f} modules of 30W")
    ...
    print(f"PV energy curtailed (Clipping) (Wclipping): {wcurtail.X:,.2f} kWh/year")
