...
eff_c  = 0.93                     # BESS charge inverter efficiency
eff_d  = 0.97                     # BESS discharge inverter efficiency
DoD    = 0.90                       # Depth of Discharge
PmaxF   = 600                      # límit frontier export/import
er = 1.0 # (exchange rate 1 Eur = 1.1 Eur, average 2024)
BoP = 0  # Balance of Plant, Eur 
Sc=1.0 # Soft costs, %
# OaMpv = 12.5  #Eur/kWp
# OaMbess = 5.9    #Eur/kWp
# CAPEX_pv = 388 #Eur/kWp
# CAPEX_BESS = 185 #Eur/kWh
# CAPEX_inverter = 48 #Eur/kWp
OaMpv = 0  #Eur/kWp
OaMbess = 0    #Eur/kWp
CAPEX_pv = 620 #Eur/kWp
CAPEX_BESS = 200 #Eur/kWh
CAPEX_inverter = 80 #Eur/kWp
i=5/100 #annual discount rate (Lazard)
n=20 #project life span
e=0/100 # escalation rate
...
# ── Capacity charges in Spain (Eur/kW-year)
...
m.addConstr(PinverterBESS == 400, name='r11')
m.addConstr(Ppvinst    == 569, name='r12')
m.addConstr(Pbmax[1]  == PbmaxP, name='r13')
m.addConstr(Pbmax[2]  == PbmaxP, name='r14')
m.addConstr(Pbmax[3]  == PbmaxP, name='r15')
m.addConstr(Pbmax[4]  == PbmaxP, name='r16')
m.addConstr(Pbmax[5]  == PbmaxP, name='r17')
m.addConstr(Pbmax[6]  == PbmaxP, name='r18')
#m.addConstr(PinverterPV == Plinst+PmaxF+PinverterBESS)   
m.addConstr(PinverterPV == 1.0*Ppvinst)   
m.optimize()
...
if m.Status == GRB.OPTIMAL:
    ...
    print(f"Operational Benefit: {Benefit.X:,.2f} Eur/year")
    print(f"Operational Expenditure with the project (OPEX): {OPEX.X:,.2f} Eur/year")
    print(f"Operational Expenditure without the project (OPEX_0): {OPEX0.X:,.2f} Eur/year")
    print(f"Operational Savings (Savings): {Savings.X:,.2f} Eur/year")
    print(f"Energy expenses with the project (Eb): {Eb.X:,.2f} Eur/year")
    print(f"Energy expenses without the project (Eb0): {Eb0.X:,.2f} Eur/year")
    print(f"Energy earnings of the project (Es): {Es.X:,.2f} Eur/year")
    print(f"Capacity Charges (CP): {CapacityP.X:,.2f} Eur/year")
    print(f"Capacity Charges without the project (CP0): {CapacityP0.X:,.2f} Eur/year")
    print(f"Capital Expenditure (CAPEX): {Investment0.X:,.2f} Eur")
    print(f"BESS battery cost: {BESSbatteryCost:,.2f} Eur")
    print(f"BESS inverter cost: {BESSinverterCost:,.2f} Eur")
    print(f"PV inverter cost: {PVinverterCost:,.2f} Eur")
    print(f"Soft Costs: {Investment0.X*(Sc-1):,.2f} Eur")
    print(f"PV system cost: {PVsystemCost:,.2f} Eur")
    print(f"Net Present Value: {npv_var.X:,.2f} Eur  (Objective)")
    print(f"Project Cash Flow: {CF:,.2f} Eur/year")
    print(f"Internal Rate of Return:      {TIR:,.2f} %")
    print(f"Pay Back Time:     {NPER:,.2f} years")
    print(f"Simple Pay Back Time:    {NPERaprox:,.2f} years")
    print(f"Net LCOE (include earnings and savings):     {LCOEnet:,.2f} Eur/MWh")
    print(f"Gross LCOE: (only CAPEX and OPEX)    {LCOEgross:,.2f} Eur/MWh")
